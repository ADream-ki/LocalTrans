...
use super::events::{
    ErrorCode, EventBroadcaster, HistoryItem, HistoryManager, PipelineEvent, PipelineState,
    TranscriptionSegment, TranslationInfo,
};
...
impl LatencyProfile {
    pub fn parse(value: &str) -> Option<Self> {
        match value.trim().to_ascii_lowercase().as_str() {
            ...
            "high-accuracy" | "high_accuracy" | "high" | "accuracy" => Some(Self::HighAccuracy),
            _ => None,
        }
    }
}
...
pub struct PipelineConfig {
    ...
    /// ASR engine selector. Legacy ids such as "whisper"/"sensevoice"/"vosk" are
    /// currently routed to the existing streaming ASR runtime for compatibility.
...
impl RealtimePipeline {
    ...
    pub fn new(app_handle: AppHandle, config: PipelineConfig) -> Result<Self> {
        ...
        let translator: Box<dyn MtAdapter> = match engine.as_str() {
            "loci" => {
                ...
                let model_path =
                    crate::loci_runtime::find_default_loci_model().ok_or_else(|| {
                        anyhow!(
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        *self.audio_capture.lock().await = Some(audio_capture);
        ...
        let (asr_engine, asr_chunk_size) = create_pipeline_asr_adapter(
            &self.config,
        ...
        if self.config.bidirectional {
            if let Some(peer_input_device) = self.config.peer_input_device.as_deref() {
                ...
                *self.peer_audio_capture.lock().await = Some(peer_capture);
                ...
                let (peer_asr_engine, _) = create_pipeline_asr_adapter(
                    &self.config,
            ...
            loop {
                    ...
                    if let Some(result) = asr_result {
                        match result {
                            StreamingResult::Partial(partial) => {
                                ...
                                let should_stream_translate = ptxt.len()
                                    >= config.stream_translation_min_chars
                                    && last_stream_translation_emit.elapsed()
                                        >= Duration::from_millis(
                                            config.stream_translation_interval_ms,
                                        )
                                    ...
                                    match translation_result {
                                        Ok(translation) => {
                                            ...
                                            if config.tts_enabled
                                                ...
                                                && translation.text.trim().len()
                                                    >= config.stream_tts_min_chars
                                                && last_stream_tts_emit.elapsed()
                                                    >= Duration::from_millis(
                                                        config.stream_tts_interval_ms,
                                                    )
                                        ...
                                        Err(err) => {
                                            if last_translation_error_emit.elapsed()
                                                >= Duration::from_secs(5)
                                            {
                                                let _ = broadcaster
                                                    .emit_error(ErrorCode::TranslationFailed, err);
                                ...
                                if !transcription.text.trim().is_empty() {
                                    ...
                                    match translation_result {
                                        Ok(translation) => {
                                            ...
                                            let _ = session_bus::append_history(
                                                &session_bus::SessionHistoryItem {
                                                    id,
                                            ...
                                            if config.bidirectional {
                                                ...
                                                if let Some(reverse) = reverse_translation {
                                                    ...
                                                    let _ = broadcaster.emit(
                                                        &PipelineEvent::BidirectionalTranslation {
                            ...
                            StreamingResult::Error(msg) => {
                                ...
                                let _ =
                                    broadcaster.emit_error(ErrorCode::AsrTranscriptionFailed, msg);
                                stats.lock().error_count += 1;
                            }
                        }
                    }
                ...
                if config.bidirectional {
                    ...
                    if !peer_samples.is_empty() {
                        let peer_resampled = if peer_input_sample_rate > 0
                            && peer_input_sample_rate != sample_rate
                        {
                        ...
                        while peer_chunk_buffer.len() >= asr_chunk_size {
                            ...
                            if let Some(StreamingResult::Final(peer_transcription)) =
                                peer_asr_result
                            {
...
fn create_pipeline_asr_adapter(
    config: &PipelineConfig,
    language: &str,
    sample_rate: u32,
    forced_chunk_size: Option<usize>,
) -> Result<(Box<dyn RealtimeAsrAdapter>, usize)> {
    let asr_config = AsrConfig {
        language: Some(language.to_string()),
        ..Default::default()
    };
    let mut streaming_config = StreamingConfig::new(sample_rate);
    streaming_config.vad_energy_threshold = config.vad_threshold;
    streaming_config.min_speech_duration_ms =
        (config.min_speech_duration_ms.min(u32::MAX as u64)) as u32;
    streaming_config.max_speech_duration_ms =
        (config.max_segment_duration_ms.min(u32::MAX as u64)) as u32;

    let chunk_size = forced_chunk_size.unwrap_or_else(|| {
        let vad_frame_ms = config.vad_frame_ms.clamp(10, 200);
        let from_ms = (sample_rate as usize * vad_frame_ms as usize) / 1000;
        if from_ms > 0 {
            from_ms
        } else {
            config.chunk_size
    ...
    let adapter = create_realtime_asr_adapter(&config.asr_engine, asr_config, streaming_config)?;
    Ok((adapter, chunk_size.max(1)))
}
